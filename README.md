...
`src/config.ts`의 `WATCH_TARGETS` 배열에 감시하고 싶은 조합을 여러 개 등록할 수 있습니다 (기본값: 용산 SCREENX 스파이더맨-브랜드 뉴 데이).
...
**요청 방식**: 순수 HTTP 요청(Node `fetch`)으로 CGV API를 두드렸더니 Cloudflare에 403으로 차단당해서,
Playwright로 실제 헤드리스 Chromium을 job당 한 번 띄우고 CGV 홈페이지를 방문해 정상적인 쿠키를 받은 다음,
그 브라우저 페이지 안에서 `fetch`를 실행하는 방식으로 바꿨습니다 (`src/browserSession.ts`, `src/cgvClient.ts`).
브라우저는 체크 사이클마다 새로 켜는 게 아니라 job 시작할 때 한 번만 띄우고 계속 재사용합니다.

매 체크마다 `WATCH_TARGETS`에 등록된 각 대상별로 `https://cgv.co.kr/api/v1/booking/searchSchByMov` 를
**오늘부터 `daysAhead`일치**(기본 7, 즉 오늘+7일=8일치)만 호출해서, 관 이름에 지정한 키워드(SCREENX, IMAX 등)가
들어간 회차 목록을 지금까지 알고 있던 목록과 비교합니다. 새로 생긴 회차가 있으면 그 즉시 텔레그램으로 알림을 보내고,
job이 끝날 때 상태 파일(`data/state.json`)을 한 번에 갱신해서 리포에 커밋합니다.
날짜 하나하나를 조회할 때마다 400~900ms 사이 랜덤한 텀을 두고, 요청이 하나 실패하면 그 다음엔 조금 더 오래(0.5~1초) 쉽니다
— 짧은 시간에 여러 요청을 몰아치는 것 자체가 봇 탐지에 걸리기 쉬운 패턴이기 때문입니다.

**주의**: `daysAhead`가 너무 좁으면 원하는 요일이 체크 범위 밖에 있어서 회차가 열려도 아예 알림이 안 갈 수 있습니다
(실제로 한 번 겪은 문제). 반대로 너무 넓히면 사이클당 요청 수가 늘어서 짧은 시간에 여러 요청이 몰리고, 이게 다시
차단 위험을 높일 수 있습니다 (이것도 실제로 겪음). 요청 개수와 필요한 날짜 범위 사이에서 적당히 조절하세요.

**자동 종료**: `src/config.ts`의 `expiresAt`(기본 2026-08-15) 지나면 체크를 자동으로 멈춥니다.
계속 쓰고 싶으면 이 날짜만 뒤로 미루면 됩니다. 예매 성공했거나 더 이상 필요 없으면 그냥 냅두면 알아서 멈춥니다.

**실패 알림**: 두 종류가 있습니다.
- 한 번의 실행(job) 내내 CGV 요청이 전부 실패하면(완전 차단 또는 API 구조 변경 가능성) 경고를 보내고 그 실행에서는
  상태 파일을 갱신하지 않습니다.
- 전부는 아니어도 그 실행에서 요청의 40% 이상이 실패하면(부분 차단 의심) 별도로 경고를 보냅니다. 완전 실패만 감지하면
  "일부 날짜만 계속 막혀서 그 날짜 새 회차만 조용히 놓치는" 상황을 놓칠 수 있어서 추가한 안전장치입니다.

두 경우 다 같은 경고가 스팸처럼 계속 오지 않도록 1시간에 한 번만 보냅니다.

**상태 파일 커밋 충돌**: `data/state.json`은 매 실행마다 통째로 다시 써서 커밋하는 파일이라, 실행이 겹치면
(스케줄과 수동 실행이 겹치는 등) push가 거부되고 텍스트 병합 충돌이 날 수 있습니다. 이 경우 워크플로우는
rebase로 억지로 맞추려 하지 않고 **원격(최신) 쪽 내용을 우선**해서 병합하고, 그래도 안 되면 원격 기준으로
그냥 맞춰버립니다 — 이번 실행에서 찾은 회차는 이미 텔레그램 알림이 나간 뒤라, 상태 파일 반영이 한 번 빠져도
다음 실행에서 다시 감지되는 정도라 안전합니다.

로컬에서 직접 커밋/푸시할 때는 `data/state.json`을 건드리지 않는 게 좋습니다 (봇이 계속 갱신하는 파일이라
로컬 사본은 금방 오래되고, `git add -A`로 같이 커밋하면 위와 비슷한 충돌이 날 수 있습니다).
...
- "새로 열린 회차 없음" 이 뜨면 정상 동작 중인 겁니다.
- "CGV API가 403을 반환했습니다" 같은 에러가 계속 뜨면 Playwright로도 여전히 차단된 것이니, 저한테 로그 붙여넣어 주세요.
- 배열을 못 찾는 에러가 나면 응답 구조가 예상과 달라서 그런 것이니, 실제 응답 원문을 공유해주세요.

전부 실패하면 텔레그램으로 경고가 오니, 그걸 기다려도 됩니다.

## 테스트

날짜 계산(`src/dates.ts`)과 키워드 매칭(`src/matching.ts`) 같은 순수 로직만 가볍게 커버합니다
(네트워크나 브라우저 실행 없음, 추가 패키지 설치도 없음 — Node 내장 테스트 러너 사용).
